	...
	if (status === "sent") {
		return (
			<div role="status" className="flex flex-col gap-3">
				<p className="text-text-strong text-[20px] font-bold sm:text-[24px]">{resume.sent.title}</p>
				<p className="text-text-secondary text-[15px] leading-[1.6] sm:text-[16px]">
					<span className="text-text-strong font-semibold break-all">{sentTo}</span> {resume.sent.detail}
				</p>
				<button
					type="button"
					onClick={() => {
		...
		<form noValidate onSubmit={handleSubmit} className="flex flex-col gap-6">
			<div className="flex flex-col gap-5">
				<Field
					id={`${uid}-name`}
					ref={nameRef}
					label={resume.fields.name.label}
					placeholder={resume.fields.name.placeholder}
					value={name}
					error={errors.name}
					disabled={sending}
					autoComplete="name"
				...
				<Field
					...
					type="email"
					inputMode="email"
					label={resume.fields.email.label}
					placeholder={resume.fields.email.placeholder}
					value={email}
					error={errors.email}
					disabled={sending}
					autoComplete="email"
			...
			{status === "failed" ? (
				<p role="alert" className="text-text-strong text-[14px] leading-[1.6]">
					{resume.errors.send}{" "}
					<a
						href={`mailto:${resume.direct.address}`}
						className="border-text-strong font-semibold break-all underline-offset-4 hover:border-b"
					>
						{resume.direct.address}
					</a>
				</p>
			) : null}

			<button type="submit" disabled={sending} aria-busy={sending} className={BUTTON}>
				{sending ? resume.submitting : resume.submit}
			</button>

			<p className="text-text-secondary text-[14px] leading-[1.6]">{resume.privacy}</p>
		</form>
	);
}
...
function Field({ id, ref, label, error, onChange, type = "text", ...rest }: FieldProps) {
