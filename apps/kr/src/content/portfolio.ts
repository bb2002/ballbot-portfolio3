	...
	blocks: [
		{
			...
			items: [
				...
				{ emphasis: "월 1,000만 원을 넘기던 AWS 비용을", detail: "30% 절감", href: "/experience/aws-cost" },
				{ emphasis: "메모리 32GB 를 다 쓰고 멈추던", detail: "API 서버 정상화", href: "/experience/api-memory" },
			],
		},
		{
			value: "2 Awards",
...
const COSMO_SCREENS: readonly Media[] = [
	...
	},
	{
		...
		height: 763,
		caption: "개발자 API",
		alt: "개발자 API 소개 화면. 자료 업로드부터 요약노트·퀴즈 조회까지의 엔드포인트를 터미널 모양 카드에 적어두었다",
	},
];
...
const ENQOR_SCREENS: readonly Media[] = [
	...
	},
	{
		...
		height: 2400,
		caption: "대화 요청",
		alt: "대화 요청 시트. 보유 클랩과 초당 연결 비용을 보여주고 영상이나 음성 중 하나를 고르게 한다",
	},
	{
...
const HEALIX_SCREENS: readonly Media[] = [
	...
	},
	{
		...
		height: 652,
		caption: "기획 배경",
		alt: "기획 배경 슬라이드. 통증을 직접 검색해보는 화면과 꾸준히 이어지는 검색량 그래프로 문제를 짚는다",
	},
	{
...
const SCREEN_TRANSLATOR_SCREENS: readonly Media[] = [
	{
		...
		note: "휴대폰을 흔들면 즉시 화면을 캡처하고, 이미지를 OCR 해 텍스트를 추출하고, 번역기로 번역해 사용자에게 오버레이 형태로 표시해주는 번역 서비스입니다.\n\n구글 번역 API 로 번역을 제공했으나, API 키 관리 관련 지식이 없어 클라이언트 앱에 그대로 키를 저장했습니다. 1,000여 명이 다운로드했을 즈음 키가 유출되었고, 하루에 수십만 원에 달하는 서버비가 발생한 것을 보고 즉시 서비스를 내려버린 해프닝이 있었습니다.",
	},
	{
	...
	},
	{
		...
		width: 560,
		height: 996,
		alt: "감도 설정 화면. 얼마나 세게 흔들어야 인식할지 슬라이더로 맞추고 그 자리에서 흔들어 시험해본다",
	},
];
...
const COSMO_STEP_SCREENS = {
	downloader: {
		...
		height: 588,
		caption: "코스모스 다운로더",
		alt: "학교 강의 사이트의 동영상 뷰어. 익스텐션이 상단 바 오른쪽에 붙인 다운로드 버튼에 빨간 동그라미를 쳐두었다",
	},
	summary: {
...
const COSMO_FRAMES: Media = {
	...
	width: 2400,
	height: 640,
	caption: "변화가 없는 구간을 묶어 오래 머문 화면만 읽는 흐름",
...
const COSMO_PIPELINE: Media = {
	...
	width: 2400,
	height: 640,
	caption: "한 번에 시키지 않고 나눠 맡기는 파이프라인",
	...
	steps: [
		...
		},
		{
			...
			paragraphs: [
				...
				"그래서 웹 사이트만으로 동작하도록 대대적으로 고쳤습니다. 학교 사이트 로그인 정보와 강좌 URL 을 입력받으면 Cloudflare Browser 로 직접 로그인하고, 패킷을 탐지해 영상을 찾아내는 구조입니다. 로그인 정보는 저장하지 않게 만들었습니다. 소프트웨어는 복잡해지고 버그도 늘었지만, 익스텐션을 없애니 사용성은 비교할 수 없이 나아졌습니다.",
			],
		},
		{
			period: "2026.09",
		...
		items: [
			{
				...
				body: [
					...
					"STT 결과와 OCR 결과를 하나의 타임라인으로 병합합니다. 마지막으로 Worker 에 이 타임라인을 전달하고, 사전에 튜닝된 프롬프트로 요약노트를 생성해냅니다.",
				],
			},
			{
				...
				body: [
					...
					"업로드된 파일은 estimator 가 검사합니다. 깨지거나 잘못된 파일이 아닌지 확인하고 문제가 없으면 샘플링해 비용을 예측합니다. 사용자는 예측된 비용을 결제해야 노트 생성이 시작됩니다. 비록 예측된 금액보다 분석해야 할 양이 많으면 손해를 볼 수 있지만, 평균 비용에 배수를 곱하게 하여 이 문제를 최소화했습니다.",
					"이 구조가 그대로 요금제로 만들어졌습니다. 2시간 영상에 약 1,300원이 듭니다. 타사 서비스가 3시간 영상을 분석하는 데 월 7,900원을 요구하니, 시험 기간에 몰아 쓰는 학생들 대상으로는 매우 합리적인 제도입니다. 인프라 또한 사용한 만큼만 청구되는 구조이기에 유지 가능한 시스템입니다.",
				],
			},
			{
				title: "플래그십 모델을 고효율 모델로 바꾸면서 노트 품질은 지켜내기",
				body: [
					"개발 초기에는 클로드 같은 플래그십 모델을 사용했습니다. 프롬프트 튜닝이나 타임라인을 크게 정리하지 않아도 만족스러운 결과가 나왔지만, 노트 한 편을 생성하는 데 돈이 너무 많이 들었습니다. 2시간짜리 영상을 클로드로 요약해주고 1,300원만 받는 것은 자선단체가 아니면 어려운 일입니다.",
					COSMO_PIPELINE,
					...
					"컨텍스트 때문에 품질이 떨어지는 문제는 해결했지만, 사고 능력의 차이가 남았습니다. 그래서 클로드 Fable 같은 플래그십 모델과 지금 파이프라인의 결과를 비교하는 하네스를 만들어 정기적으로 실행하고 있습니다. 비록 Fable 만큼의 성능이 나오지는 못하지만, 비용 대비 좋은 품질이 나오고 있습니다.",
				],
			},
		],
	},
...
const ENQOR_FIGURES = {
	...
	balance: {
		...
		height: 1040,
		caption: "통화 세션의 잔액",
		alt: "잔액 처리 구조도. 통화가 시작되면 DB 의 캐시를 잠그고 Redis 세션으로 복제한다. 잔액은 매초 줄고, 충전하면 오르고, 선물하면 내려간다. 남은 시간은 잔액에서 계산해 클라이언트에 내려주고, 통화가 끝나면 DB 에 다시 쓴다",
	},
	docs: {
...
const enqorStory: ProjectStoryContent = {
	...
	motivation: [
		"요즘은 고민이 생기면 ChatGPT 에게 먼저 물어봅니다. 그런데 정작 마음이 복잡할 때는 AI 특유의 영혼 없는 대화에 금세 지치고, 의료나 입시처럼 책임이 따르는 상담에서는 AI 의 답을 그대로 믿기도 어려웠습니다. 몇몇 대화는 아직 사람과 해야 합니다.",
		"의사 같은 전문가부터 연애를 오래 해본 사람까지, 각자 자신 있는 분야로 다른 사람의 고민을 들어주고 수익을 만드는 자리가 필요하다고 봤습니다. 대화 플랫폼을 개발하기 위해 Align Networks 에 합류해 앱과 서버 개발을 맡고 있습니다. 다만, 엔코르는 사람이 직접 프로그래밍하지 않았습니다. 기획서와 디자인을 AI 에이전트가 읽을 수 있게 만들고, 에이전트가 내놓은 결과물을 검증하는 환경을 만드는 게 제 일이었습니다.",
	],
	architecture: {
		label: "Architecture",
		items: [
			{
				...
				body: [
					...
					"다음 문제는 매초 정산이라는 요구사항입니다. 처음에는 전체 통화 시간에서 불안정했던 시간을 빼고, 통화가 끝난 뒤 데이터베이스를 한 번 업데이트할 생각이었습니다. 그런데, 요구사항이 생각 이상으로 복잡했습니다. 통화 중에 캐시(앱 안의 재화, '클랩')를 충전할 수 있어야 했고, 충전한 캐시로 서로 선물도 할 수 있으며, 통화 가능 시간이 20초 미만으로 줄면 통화가 곧 끝난다고도 알려줘야 합니다. 충전, 소비, 조회가 모두 실시간으로 이뤄져야 하므로 남은 캐시를 읽기 쓰기가 가벼운 어딘가에 저장하고 처리하는 게 간단하겠다고 판단했습니다.",
					ENQOR_FIGURES.balance,
					"통화가 시작되면 발신자 캐시를 수정할 수 없게 만듭니다. 현재 캐시를 읽어 Redis 의 통화 세션에 복제해두면 이 값이 발신자가 쓸 수 있는 캐시의 원본이 됩니다. 매초 이 값을 줄여 정산하고, 충전이나 선물이 발생하면 이 값을 계속 업데이트합니다. 충전은 결제 기록을 남겨야 하므로 DB 에 추가로 남깁니다. 남은 통화 시간도 이 값을 바탕으로 계산해 클라이언트에 내려줍니다. 통화가 끝나면 DB 에 남은 잔액을 업데이트합니다. 충전·선물을 빼면 통화 한 건의 DB 조작은 시작과 끝 두 번뿐이라, 복잡한 로직을 직관적으로 구현할 수 있었습니다.",
				],
			},
			{
				title: "AI 의 응답을 사람 병목 없이 검증하기",
				body: [
					"기획팀이 작성한 파일은 PDF 로 된 기획서와, Figma 로 작성된 디자인 파일입니다. 사람이 읽고 수정하기에는 편하지만 에이전트에게 그대로 제공하면 어느 Figma 노드에 어떤 설명이 들어 있는지 알 수 없습니다. 그래서 Figma 노드와 PDF 페이지를 직접 짝지어 Markdown 으로 작성했고, 곳곳에 메모도 적어봤습니다.",
					ENQOR_FIGURES.docs,
					"이렇게 완성한 약 800줄짜리 기획서를 가지고 AI 에이전트에게 프로젝트를 끝까지 완성하라고 했습니다. 30여 분 만에 프로젝트 하나가 나왔고, 실행해보니 그 결과는 엉망이었습니다. 어느 정도 예상은 했지만 실행조차 안 될 줄은 몰랐고, 겨우 고쳐 로그인 화면에 진입하니 Figma 에 정의된 디자인은 전혀 반영되어 있지 않았습니다. 로그인 같은 기능도 전부 모킹으로 구현된 순 엉터리 프로젝트였습니다.",
					"이 문제를 고문님께 털어놓았는데, '하네스 엔지니어링'이라는 개념을 소개해주셨습니다. 인터넷에 검색해보니 'AI 가 올바른 행동을 할 수 있게 환경을 만드는 것'이라는 모호한 답변만이 나왔습니다. 개념이 완전히 이해되지는 않았지만 고문님 조언을 듣고 훅, 비판적 리뷰어, 테스터를 하나씩 세팅하다 보니 어느새 저만의 하네스가 완성되어 있었습니다.",
					...
					"이 문제는 expo web 으로 해결했습니다. 먼저 구현하려는 화면을 expo web 으로 퍼블리싱하면, headless 브라우저를 열어 결과를 어렵지 않게 확인할 수 있습니다. 창 크기만 바꾸면 모바일·태블릿 해상도에서 깨지는 영역이 있는지도 바로 보입니다. 이렇게 웹에서 검증이 끝난 코드를 앱으로 옮기는 순서로 바꾸니 평가가 훨씬 싸졌습니다. Figma 와 웹으로 퍼블리싱된 화면이 얼마나 비슷한지는 비판적 에이전트가 판정하되, 아이콘을 직접 생성하지 못하도록 규칙을 넣었고, Figma MCP 에 문제가 생겨 동작하지 않는 경우 하네스가 아예 멈추도록 하는 preflight 절차도 추가했습니다. 그 결과 간단한 화면은 거의 완벽한 수준으로, 복잡한 화면도 70% 정도의 완성도로 나왔습니다.",
				],
			},
		],
	},
	retrospective: {
		label: "Retrospective",
		paragraphs: [
			"이번 경험에서, AI 에게 좋은 자료와 환경을 주면 그만큼의 결과가 나온다는 것을 알았습니다. 그리고 AI 가 만들어낸 결과도 '어떻게' 검증하느냐보다는 '어디까지' 검증할지를 생각하는 방향이 옳다고 느꼈습니다.",
			"금융, 우주 산업, 방위 산업처럼 사고 한 번이 참사인 도메인은 모든 코드를 전부 꼼꼼히 검증해야 할 것입니다. 하지만 이번 프로젝트처럼 대부분은 그렇지 않습니다. 개발자는 AI 가 지금 무슨 일을 하고 있고, 이번 일이 끝나면 어디까지 검증해야 하고, 어떤 파일들이 바뀌길 기대하는지 정도만 알면 충분하다고 생각합니다. 그 안에서도 단순한 작업을 AI 가 하고 있다면 몇 번 눌러보고 테스트를 마칠 수도 있고, 핵심 로직이면 설계까지 꼼꼼히 논해야 합니다.",
...
const healixStory: ProjectStoryContent = {
	...
	facts: [
		...
		{ term: "담당", detail: "백엔드 리드" },
		{ term: "주요 기술", detail: "ChatGPT API, NestJS" },
	],
	motivation: [
		"사람들은 어디가 아플 때 어느 병원에 가야 할지 모르는 경우가 많습니다. 당장 생각해봐도 열이 나는 감기에 걸리면 내과를 가야 할지 이비인후과를 가야 할지 모르겠고, 엉덩이 같은 생소한 부위가 아파오면 그 고민은 더더욱 커집니다. 과거에는 인터넷에서 검색해 적절한 과를 찾고, 지도에서 병원을 찾아야 했습니다. HEALIX 는 이 불편함을 해결하고자 개발되었습니다.",
	],
	video: {
		label: "Video",
...
const waterflakeStory: ProjectStoryContent = {
	...
	motivation: [
		"10여 년 전, 마인크래프트라는 게임에 빠져 모두와 함께할 수 있는 서버를 열어보는 게 가장 큰 목적이었습니다. 컴퓨터는 전혀 모르는 초등학생이 블로그 글만 읽고 VPS 서버를 대여해 리눅스 환경에서 게임 서버를 실행했습니다. 그리고 복잡한 네트워크 설정까지 해 마침내 서버를 열었습니다. 그때의 성취감은 평생 잊을 수 없는 기억입니다. 그래서 '만약 복잡한 과정 없이 쉽게 게임 서버를 열 수 있게 도와주는 도구가 있으면 어떨까?'라는 생각에서 시작했습니다.",
	],
	architecture: {
		label: "Architecture",
		items: [
			{
				title: "SRV 레코드로 서브도메인이 특정 IP 의 포트를 가리키도록 자동 설정하기",
				body: [
					WATERFLAKE_ARCHITECTURE,
					"사용자가 WATERFLAKE 웹 화면에서 터널을 생성하면 Cloudflare API 를 통해 SRV 레코드를 생성합니다. 그리고 터널링을 담당하는 가상 머신에 랜덤한 포트로 터널링 서버를 띄웁니다. 터널링 서버는 두 개의 연결을 받는데, 한 개는 SRV 레코드를 통해 들어오는 연결, 한 개는 마인크래프트 서버 쪽으로 나가는 연결입니다. 이 두 연결을 파이프해 외부에서 들어오는 패킷이 자연스럽게 마인크래프트 서버로 나갈 수 있습니다. 사용자에게는 도메인 주소만 입력하면 연결되는 서버처럼 보입니다.",
				],
			},
			{
				title: "게임 서버와 터널링 서버 사이를 TCP 소켓으로 터널링하기",
				body: [
...
export const projects: ProjectsContent = {
	...
	featured: [
		...
		},
		{
			period: "2026 ~ 현재",
			title: "enqor",
			description: "실시간 영상·음성으로 전문가와 1:1 상담하는 서비스입니다. 초 단위 통화 정산과, AI 에이전트의 결과물을 사람 병목 없이 검증하는 하네스를 만들었습니다.",
	...
	archive: [
		{
			period: "2024",
			title: "HEALIX",
			description: "증상을 바탕으로 가장 가깝고 적절한 병원을 추천하는 서비스입니다. 공공데이터포털의 병원 정보와 앱 내 GPS 데이터를 AI 에 전달하게 만들었습니다.",
...
const BIGINT_TABLE: Media = {
	...
	alt: "luckybox 테이블 그림. id 열이 INT 로 선언된 채 19억 건이 쌓여 있고, 위쪽 대부분의 행은 만료되어 읽히지 않으며 아래쪽 최근 한 달치 행만 실시간으로 쓰인다. 오른쪽 게이지는 INT 최댓값 21억 가운데 19억까지 찬 상태로, 남은 여유가 두 달 정도임을 보여준다",
};
...
export const experience: ExperienceContent = {
	...
	highlights: [
		...
		},
		{
			...
			body: [
				{ heading: "배경" },
				"앞선 마이그레이션으로 시간을 번 직후부터 다음 작업을 준비했습니다. 먼저 앱이 ID 를 정수로 다루던 것을 String 으로 바꿨고, 그 배포가 사용자의 99% 에 닿을 때까지 기다렸습니다. 마침 운영팀에서 랜덤 상자의 확률 조정 기능을 파악하기 어려우니 리뉴얼하자는 요청이 왔고, 대표님께서도 승인해주셔서 드디어 리팩토링에 착수할 수 있었습니다. 상자 서비스의 /luckybox 엔드포인트는 그때 분당 1,500건 안팎의 요청을 받고 있었습니다.",
				...
				"새로 생긴 정책으로, 한 사용자는 최대 20개의 상자만 열 수 있다는 제약도 있었습니다. 이 또한 상자 배열의 length 만 세면 깔끔하게 해결할 수 있었습니다.",
		...
		},
		{
			slug: "aws-cost",
			year: "2022",
			title: "월 1,000만 원을 넘기던 AWS 비용을 30% 절감",
			subtitle: "값싸고 성능 좋은 Graviton 인스턴스로 옮기고, 인스턴스 증감 규칙을 다시 잡았습니다.",
			body: [
				{ heading: "배경" },
				"타임스프레드는 서버 비용으로 매달 1,000만 원 넘게 쓰고 있었습니다. 팀장님께서 이 비용을 줄일 방법을 찾아보자고 하셨습니다.",
				...
				"며칠 뒤에는 CPU 사용량 데이터를 보고 시간대별 스케일 아웃 규칙을 조정했습니다. 피크 타임에 최대 12대까지 늘어나던 인스턴스를 8대로 줄였고, 야간에는 1대만 돌게 했습니다. 인스턴스 교체와 스케일 규칙 조정을 합쳐 비용을 30% 정도 줄일 수 있었습니다.",
			],
		},
		{
			slug: "api-memory",
			year: "2022",
			title: "메모리 32GB 를 다 쓰고 멈추던 API 서버 정상화",
			...
			body: [
				...
				{ heading: "원인과 해결" },
				"앱 푸시 발송을 병렬로 바꿔둔 코드가 문제였습니다. 병렬 처리를 위해 스레드를 만드는데, 스레드 안에서 exception 이 발생하면 그 스레드가 회수되지 않고 그대로 남아 메모리 누수가 쌓이고 있었습니다. 스레드 내부의 처리 로직에 try/catch 를 붙여 배포했고, 그 뒤로 모든 인스턴스가 1GB 정도만 쓰는 정상 상태로 돌아왔습니다.",
...
export const certificates: CertificatesContent = {
	...
	items: [
		...
		},
		{
			...
			description: "전국에서 1,000여 명만 선발되는 국가우수장학금(이공계)을 받았습니다.",
		...
		},
		{
			...
			detail: "당근마켓 수료증",
			description: "당근마켓이 20여 명만 뽑는 해커톤에 선발되었습니다.",
			...
			events: [
				{
					...
					featured: true,
					detail: "VPS 를 빌려 서버를 열고, 최고 동시접속자 120명을 기록했습니다.",
				},
				{
					year: "2015",
					title: "초등학교 졸업",
					detail: "**장래희망**으로 적은 컴퓨터 프로그래머의 꿈이 지금까지 이어지고 있습니다.",
				...
				},
				{
					...
					title: "**택스팩 커뮤니티** 개발",
					featured: true,
					detail: "PHP 로 동작하는 소모임 서비스를 개발해 가입자 100여 명을 모았습니다.",
				...
				},
				{
					...
					featured: true,
					detail:
						"휴대폰을 흔들면 화면 위에 번역 오버레이를 띄워주는 앱을 개발해 다운로드 1,000건과 수익화를 경험했습니다.",
			...
			events: [
				{
					year: "2022",
					title: "**메모리 32GB 를 다 쓰고 멈추던** API 서버 정상화",
					links: [{ name: "메모리 32GB 를 다 쓰고 멈추던", href: "/experience/api-memory" }],
				},
				{
					year: "2022",
					title: "**월 1,000만 원을 넘기던 AWS 비용을** 30% 절감",
					links: [{ name: "월 1,000만 원을 넘기던 AWS 비용을", href: "/experience/aws-cost" }],
				},
				{
					year: "2023",
					title: "**19억 건의 테이블을** 점검 시간 1시간 내에 마이그레이션",
					featured: true,
					links: [{ name: "19억 건의 테이블을", href: "/experience/bigint-migration" }],
				},
				{
					year: "2023",
					title: "**분당 1,500건의 요청을 받는** 서비스 리팩토링",
					featured: true,
					links: [{ name: "분당 1,500건의 요청을 받는", href: "/experience/realtime-redesign" }],
				},
			],
		},
		{
			...
			events: [
				...
				},
				{
					year: "2025",
					title: "국가우수장학(이공계)",
					detail: "전국에서 1,000여 명만 선발되는 국가우수장학금(이공계)을 받았습니다.",
				},
				{
					year: "2025",
					title: "일본 돗토리대학교 교환학생",
					featured: true,
					detail: "일본에서 한 학기를 지내며 다도부에서 전통 문화를 배웠습니다.",
				},
				{
					year: "2026",
